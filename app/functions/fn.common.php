...
function fn_print_r()
{
    ...
    if ($console) {
        ...
        fn_echo(
            '<ol style="font-family: Courier; font-size: 12px; border: 1px solid #dedede; background-color: #efefef; float: left; padding-right: 20px;" '
            . 'class="cs-debug-printr">'
        );
        foreach ($args as $v) {
...
function fn_tools_update_status(array $params)
{
    ...
    if (Registry::get('runtime.company_id')) {
        ...
        // Add company_id permission check via related tables
        if (fn_allowed_for('MULTIVENDOR')) {
            $company_joins_schema = fn_get_schema('tools', 'update_status_company_joins');
            if (!empty($company_joins_schema[$params['table']])) {
                $company_join_data = $company_joins_schema[$params['table']];
                $company_joins = [];
                $current_table = $params['table'];

                if (!empty($company_join_data['join_via_stack'])) {
                    foreach ($company_join_data['join_via_stack'] as $join_data) {
                        $company_joins[] = db_quote(
                            'LEFT JOIN ?:?f ON ?:?f.?f = ?:?f.?f',
                            $join_data['table'],
                            $join_data['table'],
                            $join_data['join_by_from'],
                            $current_table,
                            $join_data['join_by_to']
                        );
                        $current_table = $join_data['table'];
                    }
                }

                $company_joins[] = db_quote(
                    'LEFT JOIN ?:?f ON ?:?f.?f = ?:?f.?f',
                    $company_join_data['company_join_table'],
                    $company_join_data['company_join_table'],
                    $company_join_data['join_by_from'],
                    $current_table,
                    $company_join_data['join_by_to']
                );
            }
        }

        if (in_array('company_id', $cols) || isset($company_joins)) {
            ...
            $join = '';
            $conditions = [];
            if (!empty($company_joins) && !empty($company_join_data['company_join_table'])) {
                $join .= implode(' ', $company_joins);
                $conditions['?:' . $params['table'] . '.' . $params['id_name']] = $params['id'];
                $company_condition = fn_get_company_condition("?:{$company_join_data['company_join_table']}.company_id");
            } else {
                $conditions[$params['id_name']] = $params['id'];
                $company_condition = fn_get_company_condition("?:{$params['table']}.company_id");
            }

            $permission = db_get_field(
                'SELECT company_id FROM ?:?f ?p WHERE ?w ?p',
                $params['table'],
                $join,
                $conditions,
                $company_condition
...
function fn_update_logo($logo_data, $company_id = null, $storefront_id = null)
{
    ...
    if (!empty($logo_data['image_path'])) {
        ...
        if (file_exists($logo_data['image_path'])) {
            ...
            if (!empty($logo_data['image_alt'])) {
                $_REQUEST['logotypes_image_data'] = [
                    [
                        'type'      => 'M',
                        'object_id' => $logo_id,
                        'image_alt' => $logo_data['image_alt']
                    ],
                ];
            } else {
                $_REQUEST['logotypes_image_data'] = [
 ...
 * @param string $type       Object type
 * @param bool   $delete_key Flag to delete key
 *
 * @return mixed object ID
 */
function fn_get_object_by_ekey($ekey, $type, $delete_key = true)
{
